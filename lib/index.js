...
import { tap, groupBy, nth, match, merge, partial, partialRight, flatten, concat, values, mapObj, map, compose as $$, filter, not } from 'ramda'
...
let path = partial(join, __dirname)
...
function create(customConfig = {}) {
  ...
  function apply(compiler) {
    compiler.plugin('emit', (c, cb) => {
      ...
      let assetTypesAssetPaths = getPublicAssets(stats.assetsByChunkName)
